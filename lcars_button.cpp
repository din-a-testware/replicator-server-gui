...
void LCARS_Button::paintEvent(QPaintEvent*) {
    ...
    bool restrictedHeight = this->property("restricted_Height").toBool();



    switch (restrictedHeight) {
    ...
    QSize Size_TopRight(maxHeight,maxHeight);
    QSize Size_BottomRight(maxHeight,maxHeight);

    QRect rect_topRightEdge(this->width()-Size_TopRight.width(),
                            0,
                            Size_TopRight.width(),
                            Size_TopRight.height()+1);

    QRect rect_bottomRightEdge(this->width()-Size_BottomRight.width(),
                               maxHeight-Size_BottomRight.height(),
    ...
    QPoint rightCenter(getRect(rect_topRightEdge,rect_bottomRightEdge,Qt::RightEdge).at(1).x()+getRect(rect_topRightEdge,rect_bottomRightEdge,Qt::RightEdge).at(1).width()-extendRight,maxHeight-1);
    ...
    int Divider_Left_Position = this->property("Divider_Left_Position").toInt()+maxHeight/4;//0;
    int Divider_Left_Width = this->property("Divider_Left_Width").toInt()+boundingRect.width();//40;
    ...
    int Divider_Second_Position = this->property("Divider_Second_Position").toInt();//0;
    int Divider_Second_Width = maxHeight/3;//this->property("Divider_Second_Width").toInt()+boundingRect.width();//40;

    QRectF Divider_Left(centerRect.x()+Divider_Left_Position,0,maxHeight*1.6,maxHeight);
    QRectF Divider_Right(centerRect.x()+centerRect.width()-Divider_Right_Position,0,-1*Divider_Right_Width,maxHeight);

    QRectF Divider_Second(centerRect.x()+Divider_Left_Position+maxHeight/8,0,Divider_Second_Width,maxHeight);
    ...
    case 0:
        ...
        Divider_Second_Color = this->property("Divider_Second_Color").toString();
    ...
    case 1:
        ...
        Divider_Second_Color = this->property("Red_Divider_Second_Color").toString();
    ...
    case 2:
        ...
        Divider_Second_Color = this->property("Active_Divider_Second_Color").toString();
    ...
    if (isDown()) {
        ...
        //! { PRESSED }
         ...
         Divider_Second_Color = this->property("Pressed_Divider_Second_Color").toString();
    ...
    Left_Text_Font.setPixelSize(maxHeight*1.4);
    ...
    Center_Text_Font.setPixelSize(maxHeight/2);
    QFont Right_Text_Font(this->font()/*this->property("Right_Text_Font").toString()*(/*"Helvetica Ultra Compressed", 30, QFont::Normal */);

    int Text_Center_Distance_Right = rect_topLeftEdge.width()/2;//this->property("Text_Center_Distance_Right").toInt(); //15;
    ...
    painter.fillRect(centerRect.x()-extendLeft,0,extendLeft,maxHeight,Edge_Left_Color);
    ...
    //! ( EXTEND THE EDGE WITH THE GIVEN SIZE )
    painter.fillRect(centerRect.width()+centerRect.x(),0,extendRight,maxHeight,Edge_Right_Color);
    ...
    painter.eraseRect(Divider_Second);
    painter.fillRect(Divider_Second,Divider_Second_Color);
    ...
    int Left_Text_Distance = this->property("Left_Text_Distance").toInt();
    //! [ -- TEXT -- ]
    ...
    painter.drawText(centerRect.x()+Divider_Left.width()-maxHeight-maxHeight/8,0,/*Divider_Left_Width+maxHeight/4*/maxHeight+maxHeight/4,maxHeight,Left_Text_VAlign|Left_Text_HAlign,this->property("Text_Left").toString()/*,&boundingRect*/);


    //qDebug() << "lt: " << Left_Text_Distance;

    //! [ RIGHT TEXT ]
    /*painter.setPen(QPen(Left_Text_Color));
    painter.setFont(Left_Text_Font);
    painter.drawText(centerRect.x(),0,Divider_Left_Width,maxHeight,Qt::AlignVCenter|Qt::AlignHCenter,"52");*/
...
QVector<QRect> LCARS_Button::getRect(QRect rect_topLeftEdge, QRect rect_bottomLeftEdge, int edge) {
    ...
    case Qt::LeftEdge:
    {
        ...
        case 1:
        {
            ...
            returnVector->append(leftRect);
                ...
                QPoint bottomLeftPoint(rect_bottomLeftEdge.width()/2-1,maxHeight);
...

    {
        ...
        case 1:
        {
            ...
            QPoint bottomLeftPoint(rect_topLeftEdge.x()+rect_topLeftEdge.width()/2,maxHeight);
            QPoint botright2(this->width()-rect_bottomLeftEdge.width()/2,maxHeight-rect_bottomLeftEdge.height()/2-1);
            QRect fillRect(bottomLeftPoint,botright2);
            returnVector->append(fillRect);
